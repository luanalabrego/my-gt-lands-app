...
export async function GET() {
  try {
    ...
    // 1) Lê todos os usuários na aba "Usuario" (colunas A:E)
    const resUsu = await sheets.spreadsheets.values.get({
      spreadsheetId,
      range: 'Usuario!A:E',
    })
    const usuarios = resUsu.data.values || []  // inclui cabeçalho

    // 2) (opcional) se precisar de outra aba, faça aqui…
    ...
    console.error('[API GET /usuario]', err)
...
export async function POST(request: Request) {
  try {
    ...
    console.error('[API POST /usuario]', err)
    return NextResponse.json({ ok: false, error: err.message }, { status: 500 })
  }
}
