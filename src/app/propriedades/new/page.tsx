  ...
  return (
    <div className="max-w-xl mx-auto">
      ...
      <button
        type="button"
        onClick={() => router.back()}
        className="mb-4 text-sm text-white bg-gray-700 px-3 py-1 rounded hover:bg-gray-600"
      >
        ← {t('Voltar')}
      </button>
      ...
      <form
        ...
        className="bg-[#2C2C2C] p-6 rounded-2xl shadow-lg space-y-6 text-white"
      >
        ...
        <fieldset className="border border-gray-700 rounded-lg p-4 space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            {(Object.keys(FIELDS) as FieldKey[])
              .filter(key => ![
                'purchaseDate','propertyNumber','parcel',
                'address','county','state',
                'squareFeet','acres','minimumLotArea',
                'zoningCode','zoningType','notesZone',
                'water','waterDesc','electricity','electricityDesc','sewer','sewerDesc'
              ].includes(key))
              .map(key => (
              <div key={key}>
                <label className="block text-xs font-medium mb-1 text-gray-300">
